...
static NSString *EYInputCorrectionLevelL = @"L";//!< L: 7%
static NSString *EYInputCorrectionLevelM = @"M";//!< M: 15%
static NSString *EYInputCorrectionLevelQ = @"Q";//!< Q: 25%
static NSString *EYInputCorrectionLevelH = @"H";//!< H: 30%

@interface EYQRCodeManager ()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIGestureRecognizerDelegate,EYCodePreviewViewDelegate>
...
@implementation EYQRCodeManager

#pragma mark - 扫描二维码/条形码
- (instancetype)initWithPreviewView:(EYQRCodePreView *)previewView completion:(nonnull void (^)(void))completion {
    
    self = [super init];
    
    if (self) {
        
        if ([previewView isKindOfClass:[EYQRCodePreView class]]) {
            _previewView = (EYQRCodePreView *)previewView;
...
- (void)stopScanning {
    ...
    [EYQRCodeManager switchTorch:NO];
    [EYQRCodeManager resetZoomFactor];
}
...
#pragma mark - EYCodePreviewViewDelegate
...
- (void)codeScanningView:(EYQRCodePreView *)scanningView didClickedTorchSwitch:(UIButton *)switchButton {
    ...
    [EYQRCodeManager switchTorch:switchButton.selected];
...
+ (UIImage *)generateQRCode:(NSString *)code size:(CGSize)size {
    ...
    CIFilter *filter = [CIFilter filterWithName:@"CIQRCodeGenerator" withInputParameters:@{@"inputMessage": codeData, @"inputCorrectionLevel": EYInputCorrectionLevelH}];
    UIImage *codeImage = [EYQRCodeManager scaleImage:filter.outputImage toSize:size];
...
+ (UIImage *)generateQRCode:(NSString *)code size:(CGSize)size logo:(nonnull UIImage *)logo {
    ...
    codeImage = [EYQRCodeManager combinateCodeImage:codeImage andLogo:logo];
